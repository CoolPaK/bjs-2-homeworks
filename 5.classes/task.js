// Задание 1.
class PrintEditionItem {
    constructor(name, releaseDate, pagesCount) {
        this.name = name;
        this.releaseDate = releaseDate;
        this.pagesCount = pagesCount;
        this.state = 100;
        this.type = null;
    }

    fix() {
        this.state *= 1.5;
    }

    set state(number) {
        if (number < 0) {
            this._state = 0;
        } else if (number > 100) {
            this._state = 100;
        } else {
            this._state = number;
        }
    }

    get state() {
        return this._state;
    }
}

class Magazine extends PrintEditionItem {
    constructor(name, releaseDate, pagesCount) {
        super(name, releaseDate, pagesCount);
        this.type = "magazine";
    }
}

class Book extends PrintEditionItem {
    constructor(author, name, releaseDate, pagesCount) {
        super(name, releaseDate, pagesCount);
        this.author = author;
        this.type = "book";
    }
}

class NovelBook extends Book {
    constructor(author, name, releaseDate, pagesCount) {
        super(author, name, releaseDate, pagesCount);
        this.type = "novel";
    }
}

class FantasticBook extends Book {
    constructor(author, name, releaseDate, pagesCount) {
        super(author, name, releaseDate, pagesCount);
        this.type = "fantastic";
    }
}

class DetectiveBook extends Book {
    constructor(author, name, releaseDate, pagesCount) {
        super(author, name, releaseDate, pagesCount);
        this.type = "detective";
    }
}

// Задание 2.
class Library {
    constructor(bookName) {
        this.name = bookName;
        this.books = [];
    }

    addBook(book) {
        if (book.state > 30) this.books.push(book);
    }

    findBookBy(type, value) {
        for (let i = 0; i < this.books.length; i++) {
            let book = Object.entries(this.books[i]);
            for (let j = 0; j < book.length; j++) {
                if (book[j][0] === type && book[j][1] === value) return this.books[i];
            }
        }
        return null;
    }

    giveBookByName(bookName) {
        let book = this.findBookBy("name", bookName);
        if (book != null) this.books.splice(this.books.indexOf(book), 1);
        return book;
    }
}

// Задание 3.
class Student {
    constructor(name) {
        this.name = name;
        this.marks = {};
    }

    addMark(mark, subject) {
        if (mark < 2 || mark > 5) {
            return; // оценка не добавится, если не входит в диапазон
        }

        if (!(subject in this.marks)) {
            this.marks[subject] = []; // добавляем новый предмет, если его нет
        }

        this.marks[subject].push(mark); // добавляем оценку по предмету
    }

    getAverageBySubject(subject) {
        if (!(subject in this.marks) || this.marks[subject].length === 0) {
            return 0; // если предмет отсутствует или без оценок, вернуть 0
        }

        const sum = this.marks[subject].reduce((acc, curr) => acc + curr, 0);
        return sum / this.marks[subject].length; // возвращаем среднюю оценку
    }

    getAverage() {
        const subjects = Object.keys(this.marks);

        if (subjects.length === 0) {
            return 0; // если нет предметов, вернуть 0
        }

        const totalAverage = subjects.reduce(
            (acc, subject) => acc + this.getAverageBySubject(subject),
            0,
        );
        return totalAverage / subjects.length; // возвращаем общую среднюю оценку
    }
}